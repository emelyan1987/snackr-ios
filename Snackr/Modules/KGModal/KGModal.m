...
@interface KGModal() <FirstCommentModalDelegate, SwipeDownCommentDelegate, shareModalDelegate, ShareReferralCodeViewDelegate, PostConfirmViewDelegate>
...
- (void) onCancel
{
    ...
    [self.delegateSwipe actionAfterUndo];
}
- (void) onDone
{
    [self closeBarInfoDialog];
    
    [self.delegateSwipe actionAfterDone];
}
...
-(void) onShareRefferalCode{
    //[self closeBarInfoDialog];
    [self.delegateShareRefferralCode shareRefferralCode];}
